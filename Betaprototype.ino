//Source Code can be found on https://github.com/zhangxu92/FYP2015_XZ
/*Beta Prototype
Change log 17/06/2015
All system functions are working as intended
Random profile selection added to profile 0
Transition modes added

Known Bugs
some displaying errors due to displaying the variable right before the value is reset
*/
...
preset_t preset1[] = {90.0, 5, 4, 90.0, 2, 2, 92.5, 25, 10};	//3 step brew
preset_t preset2[] = {90.5, 4, 3, 90.0, 4, 6 , 90.0, 5, 10, 90.0, 5, 8, 90.0, 2.5, 6};	// 5 step brew
preset_t preset3[] = {88,30, 9};	//Test mode
preset_t preset4[] = {88.5, 60, 10};	//off the shelf - 1 step brew
...
void setup() {
  ...
  PIDsetup(90, 0.05, 1.5, 100); //Set P,I,D gains and sample time
...
void loop() {
  ...
  if (mode > 0 && mode < 5) {
    ...
    if (skipcount > 2) { //skip initial readings before start to control heater to avoid overshoot

      if (setT - temp > 50) {  //force 100% while temperature difference is larger than 45, 
                                                     //reducing this value would cause overshoot 
        output = 0x1E85;  
      }

      else if ((setT - temp > 8 && activePresetStep > 0)) {
        output = 0xF43;  //force to 50% if temperature drops 8 degrees while brewing
      }
